...
export class HistorialProvider {
  ...
  constructor(private toastCtrl: ToastController, private platform: Platform, private iab: InAppBrowser, private modalCtrl: ModalController, private contacts: Contacts) { }
  ...
  abrirCodigoQR() {
    let scanData = this._historial[0];
    console.log('Historial privider');
    console.log(scanData);

    switch (scanData.tipo) {
      case 'Sitio Web':
        console.log('Pagina que se abrirar');
  ...
  crearContacto(texto: string) {
    ...
    contacto.save().then(
      () => this.crearToast('Contacto: ' + nombre + 'ha sido creado.'),
